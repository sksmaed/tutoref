...
import {
  MY_PLANS_CACHE_KEY,
  MY_LIKES_CACHE_KEY,
  LIKES_STATE_CACHE_KEY,
  buildLikesMapFromRows,
} from './cache';
...
export function usePlanManagement() {
  ...
  useEffect(() => {
    ...
    const loadMyPlans = async () => {
      // 快取先行：有快取就先顯示、背景更新；沒有才進 loading 狀態
  ...
  const handleFavoriteToggle = async (row: Row) => {
    ...
    if (currentlyLiked) {
      ...
      nextFavorites = previousFavorites.some((item) => item.id === id)
        ? previousFavorites
        : [row, ...previousFavorites];
    }
